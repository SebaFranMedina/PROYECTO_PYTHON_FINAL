...
class GestorAutores:
    ...
    def eliminar(self, autor_id: int):
        """Elimina un autor por su ID (solo si no tiene libros)"""
        autor = self.session.query(Autor).get(autor_id)
        if not autor:
            return None
        # 🚫 Bloqueo: no se puede borrar si tiene libros asociados
        if hasattr(autor, "libros") and autor.libros:
            raise ValueError("No se puede eliminar un autor con libros asociados")
        try:
